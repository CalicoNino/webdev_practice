...
public partial class ProductItem : System.Web.UI.Page
{
    ...
    protected void ConfigureComponents()
    {
        ...
        var components = Connection.GetAllComponents();
    ...
    private Component GetComputer()
    {
        ...
        return Connection.GetComputer(Int32.Parse(compId));
    }
    ...
    protected void btnAddToCart_Click(object sender, EventArgs e)
    {
        ...
        {
            Connection.GetComponent(Int32.Parse(optDisplay.SelectedItem.Value)),
            Connection.GetComponent(Int32.Parse(optDrive.SelectedItem.Value)),
            Connection.GetComponent(Int32.Parse(optRam.SelectedItem.Value)),
            Connection.GetComponent(Int32.Parse(optCpu.SelectedItem.Value)),
            Connection.GetComponent(Int32.Parse(optOs.SelectedItem.Value))
        };
    ...
    {
        var IsUserLoggedIn = loginSignUp.Text != "LOG IN/SIGN UP";
        err.Text = IsUserLoggedIn ? "" : "PLEASE LOG IN TO ADD TO CART";

        if (IsUserLoggedIn)
        {
            item.TableRowId = Connection.AddCartItemToOrder(item, loginSignUp.Text);
    ...
    private double ParseDropDownListForPrice(DropDownList dropdown)
    {
        ...
        var item = Connection.GetComponent(Int32.Parse(itemId));
